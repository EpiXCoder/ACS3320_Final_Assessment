			...
			const hue = i * (360 / swatchCount); // Distributes the hues evenly across 360 degrees
			document.querySelector(`.color-${i}`).style.backgroundColor = `hsl(${hue}, 100%, 50%)`;
			}
      ...
      const originalText = tickerTextElement.textContent;
      tickerTextElement.innerHTML = colorizeTickerText(originalText);

    //   const headerTextElement = document.getElementById('page-title');
    //   const headerOriginalText = headerTextElement.textContent;
    //   headerTextElement.innerHTML = colorizeTickerText(headerOriginalText);


      function colorizeTickerText(text) {
        const coloredText = Array.from(text).map((letter, index) => {
          const hue = index * (360 / text.length);
          return `<span style="color: hsl(${hue}, 100%, 50%);">${letter}</span>`;
        }).join('');
